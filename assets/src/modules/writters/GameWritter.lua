...
function GameWritter.objectMove(player, mover)
    ...
    if mover.type == ObjectType.MONSTER then
...
local function updateInventory(player, inventory, type)
    local packet = Packet.new(Cmd.CHAR_INVENTORY)
    if type == 3 then
        packet:writeByte(0)
...
local function updateStorage(player, inventory, type)
    local packet = Packet.new(Cmd.UPDATE_CHAR_CHEST)
    if type == 3 then
        packet:writeByte(inventory.maxSize)
        packet:writeByte(0)
        packet:writeByte(type)
        packet:writeByte(type)
...
function GameWritter.updateStorage(player)
    updateStorage(player, player.bank, 4)
    updateStorage(player, player.bank, 7)
...
function GameWritter.fireMonster(player, context)
    local cmd = context.target.type == ObjectType.MONSTER and Cmd.FIRE_MONSTER or Cmd.FIRE_PK
...
function GameWritter.leaveMap(other, playerId)
    local packet = Packet.new(Cmd.PLAYER_EXIT)
    packet:writeShort(playerId)
    other:send(packet)
end

function GameWritter.changeFlag(other, data)
    local packet = Packet.new(Cmd.PK)
    packet:writeShort(data.id)
    packet:writeByte(data.flag)
    other:send(packet)
end

function GameWritter.playerDie(other, attacker, target)
    local packet = Packet.new(Cmd.DIE_PLAYER)
    packet:writeShort(target.id)
    packet:writeShort(attacker.id)
    packet:writeByte(0)
    packet:writeByte(attacker.type)
    other:send(packet)
end

function GameWritter.updateExp(other, data)
    local packet = Packet.new(Cmd.SET_EXP)
    packet:writeShort(data.id)
    packet:writeShort(data.expPercent)
    packet:writeInt(data.gainedExp)
    other:send(packet)
end

function GameWritter.levelUp(other, data)
    local packet = Packet.new(Cmd.LEVEL_UP)
    packet:writeShort(data.id)
    packet:writeByte(data.level)
    other:send(packet)
end

function GameWritter.usePotion(other, target, type, value)
    local packet = Packet.new(Cmd.USE_POTION)
    packet:writeByte(target.type)
    packet:writeShort(target.id)
    packet:writeShort(-1)
    if target.type == ObjectType.PLAYER then
        packet:writeByte(type)
        packet:writeInt(type == 1 and target.maxMp or target.maxHp)
        packet:writeInt(type == 1 and target.mp or target.hp)
...
function GameWritter.listZone(player, zones)
    local packet = Packet.new(Cmd.UPDATE_STATUS_AREA)
    packet:writeByte(zones:size())
    zones:forEachIndexed(function(index, zone)
        packet:writeByte(zone.status)
        packet:writeByte((index == zones:size() - 1) and 3 or 0)
    end)
    zones:forEachIndexed(function(index, zone)
        packet:writeUTF(string.format("%s %d (%d/%d)", "Area", zone.id + 1, zone.players, zone.max))
    end)

    player:send(packet)
end

function GameWritter.openStorage(player)
    local packet = Packet.new(Cmd.NPC_INFO)
