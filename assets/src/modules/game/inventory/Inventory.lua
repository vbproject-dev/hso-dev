...
function Inventory:transferTo(target, item, quantity)
    ...
    if item.category == 4 or item.category == 7 then
        ...
        if not target:findById(item.id, item.category) and target:isFull() then
            return false
        end

        if not target:addFrom(item.id, item.category, quantity) then
            return false
        end
    ...
    if not target:add(item) then return false end
