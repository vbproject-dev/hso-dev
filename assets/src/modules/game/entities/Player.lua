...
function Player:ctor(data)
    ...
    local skillLevelData = ArrayList.new(data.skill or {})
    skillLevelData:forEachIndexed(function(id, level)
        local skillData = GameData.getSkillData(self.class, id)
        if skillData then
...
function Player:resetSkills()
    self.skills:forEach(function(skill)
        ...
        skill.levelData = skill.level > 0 and skill.levels:get(skill.level - 1) or nil
    end)
...
function Player:addExperience(experience)
    if experience <= 0 or self.level >= LevelSystem.MAX_LEVEL then
        return
    end
    ...
    while LevelSystem:canLevelUp(self) do
        ...
        CharacterWritter.mainCharInfo(self)
        self.zone:forEachPlayer(function(other)
            GameWritter.levelUp(other, {
                id = self.id,
                level = self.level
            })
        end)
...
function Player:getExpPercent()
    if self.level >= LevelSystem.MAX_LEVEL then
        return 0
    end

    local requiredExperience = LevelSystem:getRequiredExperience(self.level)
    if requiredExperience <= 0 then
        return 0
    end

    return math.floor((self.exp * 1000) / requiredExperience)
end

function Player:update(dt)
    self.regenTimer = self.regenTimer + dt

    if self.regenTimer < 2 then
        return
    end
...
function Player:restoreHp(value)
    self.hp = math.min(self.hp + value, self.maxHp)
    local GameWritter = require("modules.writters.GameWritter")
    self.zone:forEachPlayer(function(other)
        GameWritter.usePotion(other, self, 0, value)
    end)
end

function Player:restoreMp(value)
    self.mp = math.min(self.mp + value, self.maxMp)
    local GameWritter = require("modules.writters.GameWritter")
    self.zone:forEachPlayer(function(other)
