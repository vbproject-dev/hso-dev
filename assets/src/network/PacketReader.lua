...
function PacketReader.onChangeFlag(packet)
    return {
        type = packet:readByte()
    }
end

function PacketReader.onCharInfo(packet)
    return {
        id = packet:readShort(),
    }
end

function PacketReader.onFirePK(packet)
    return {
        skillId = packet:readByte(),
        type = packet:readByte(),
        targetId = packet:readShort(),
...
function PacketReader.onGoHome(packet)
    return {
        type = packet:readByte()
    }
end

function PacketReader.onChangeArea(packet)
    return {
        zoneId = packet:readByte()
    }
end

function PacketReader.onUpdateStorage(packet)
    return {
...
return {
    ...
    [Cmd.PK] = PacketReader.onChangeFlag,
    [Cmd.CHAR_INFO] = PacketReader.onCharInfo,
    [Cmd.FIRE_PK] = PacketReader.onFirePK,
