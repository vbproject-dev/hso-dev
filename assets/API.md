...
| Field                     | Description                                   |
| ------------------------- | --------------------------------------------- |
| `WebSocket.send(data)`    | Sends data through the WebSocket connection.  |
| `WebSocket.close()`       | Closes the WebSocket connection.              |
| `WebSocket.isConnected()` | Returns `true` when the connection is active. |
| `WebSocket.getId()`       | Returns the unique connection ID.             |
...
| `WebSocketServer.start(port)`         | Starts the WebSocket server on the specified port. |
| `WebSocketServer.stop()`              | Stops the WebSocket server.                        |
| `WebSocketServer.poll()`              | Processes pending server events.                   |
| `WebSocketServer.setHandler(handler)` | Registers callbacks for server events.             |
| `WebSocketServer.broadcast(data)`     | Sends data to all connected clients.               |
| `WebSocketServer.closeAll()`          | Closes all connected clients.                      |
| `WebSocketServer.getClientCount()`    | Returns the number of connected clients.           |
...
server:setHandler({
    ...
    onMessage = function(client, bytes)
        -- received packet
    end
})
```
...
| -------------------------- | ----------------- | --------------------------------------------------- |
| `onAccept(client)`         | `client`          | Called when a client connects to the server.        |
| `onDisconnect(client)`     | `client`          | Called when a client disconnects from the server.   |
| `onMessage(client, bytes)` | `client`, `bytes` | Called when the server receives data from a client. |

# WebSocketClient class

| Field                                 | Description                                  |
| ------------------------------------- | -------------------------------------------- |
| `WebSocketClient.new()`               | Creates a new WebSocket client.              |
| `WebSocketClient.connect(url)`        | Connects to the specified WebSocket server.  |
| `WebSocketClient.disconnect()`        | Disconnects from the WebSocket server.       |
| `WebSocketClient.send(data)`          | Sends data to the WebSocket server.          |
| `WebSocketClient.poll()`              | Processes pending client events.             |
| `WebSocketClient.setHandler(handler)` | Registers callbacks for client events.       |
| `WebSocketClient.isConnected()`       | Returns `true` when the client is connected. |
...
| `onConnect()`      | —          | Called when the client connects to the server.        |
| `onDisconnect()`   | —          | Called when the client disconnects from the server.   |
| `onMessage(bytes)` | `bytes`    | Called when the client receives data from the server. |
